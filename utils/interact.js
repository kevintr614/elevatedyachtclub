...
export const StartMint = async (mintAmount) => {
  ...
  if (!window.ethereum.selectedAddress) {
   ...
   //Already Claimed
   const claimed = await nftContract.methods.addressClaimedVIP(window.ethereum.selectedAddress).call()
   if (claimed >= 2) {
     return {
       success: false,
       status: 'Address Already Minted!'
     }
   }
  ...
  try {
    ...
    return {
	  ...
	  <p>Congratulations, you have minted {`${mintAmount}`} VIP Pass!</p>	  
